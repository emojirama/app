    ...
    for scene in scene_list:
        for i, row in enumerate(emojirama.board["scenes"][scene]["data"]):
            for j, square in enumerate(row):
                p.hset(f"emojirama___{emojirama.id}___{scene}___{i}___{j}___square", 'emoji', square['emoji']) # noqa
                p.hset(f"emojirama___{emojirama.id}___{scene}___{i}___{j}___square", 'color', square['color']) # noqa
                p.hset(f"emojirama___{emojirama.id}___{scene}___{i}___{j}___square", 'tone', square['tone']) # noqa
                p.hset(f"emojirama___{emojirama.id}___{scene}___{i}___{j}___square", 'position', str(square['position'])) # noqa
    return p.execute()
...
def get_emojirama_from_redis(emojirama):
    ...
    for scene in scene_list:
        ...
        while True:
            ...
            col_num = 0
            try:
                _ = key_hash_dict[f"emojirama___{emojirama_id}___{scene}___{row_num}___{col_num}___square"] # noqa
            except KeyError:
                break
            while True:
                try:
                    h = key_hash_dict[f"emojirama___{emojirama_id}___{scene}___{row_num}___{col_num}___square"] # noqa
                    h["tone"] = int(h["tone"])
